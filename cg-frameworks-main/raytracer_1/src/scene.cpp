...
Color Scene::trace(Ray const &ray)
{
  ...
  for (auto &light : lights)
  {
    ...
    color += std::max(N.dot(L), 0.0) * material.kd * light->color * material.color;
    if (N.dot(L) > 0)
    {
      double sPhy = std::max(reflect(-L, N).dot(V), (double)0);
      double spec = std::pow(sPhy, material.n);
      color += spec * material.ks * light->color;
    }
  }
